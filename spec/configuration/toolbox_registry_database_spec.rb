...
describe 'toolbox registry database configuration' do
  ...
  context 'when registry database user is configured' do
    ...
    it 'creates the registry database user ConfigMap' do
      ...
      expect(template.resource_exists?('ConfigMap/test-toolbox-registry-db-user')).to be true
      expect(template.dig('ConfigMap/test-toolbox-registry-db-user', 'data', 'db-user')).to include('REGISTRY_DATABASE_USER=registry_backup_user')
    end
    ...
    it 'mounts registry-db-config volume in deployment' do
      ...
      user_config = sources.find { |s| s['configMap'] && s['configMap']['name'] == 'test-toolbox-registry-db-user' }
  ...
  context 'when registry database user is not configured' do
    ...
    it 'does not create the registry database user ConfigMap' do
      expect(template.exit_code).to eq(0), "Unexpected error code #{template.exit_code} -- #{template.stderr}"
      expect(template.resource_exists?('ConfigMap/test-toolbox-registry-db-user')).to be false
    end
