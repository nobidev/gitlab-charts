...
| `database.enabled`                                       | `prefer`                                                             | Enable metadata database. The default `prefer` setting will enabled metadata database feature for new installations using bundled PostgreSQL. If you're using an external database, please explicitly set this to `true` or `false`, as additional setup is required. We recommend migrating to this new feature. Read [Metadata Database](metadata_database.md) for migration instructions. |
