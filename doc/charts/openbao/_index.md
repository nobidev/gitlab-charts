...
AWS KMS unsealing delegates the unseal operation to an [AWS KMS key](https://openbao.org/docs/configuration/seal/awskms/).
KMS handles key rotation automatically, which makes this the recommended option for AWS-based deployments.
...
with the IAM role ARN that has `kms:Encrypt`, `kms:Decrypt`, and `kms:DescribeKey` permissions on the KMS key:
...
> [!note]
> For [GitLab Geo](https://docs.gitlab.com/ee/administration/geo/) deployments, use
> [AWS multi-region replica keys](https://docs.aws.amazon.com/kms/latest/developerguide/multi-region-keys-replicate.html).
> AWS replicates key material to each region automatically, so the same key ID or alias
> resolves on all sites without additional chart configuration.
