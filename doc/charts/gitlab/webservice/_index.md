...
| `global.registry.enabled`                                     | `true`                                                          | Enable the Container Registry integration (controls the registry link in the projects menu and whether the GitLab application contacts the Registry, for example when deleting or transferring projects). Set to `false` to fully disable Container Registry features. |
...
| `shutdown.blackoutSeconds`                                    | `10`                                                            | Number of seconds to keep Webservice running after receiving shutdown. It must be shorter than `deployment.terminationGracePeriodSeconds`. Also configures the shutdown delay of the workhorse healthcheck listener if it is enabled. |
...
| `trusted_proxies`                                             | `[]`                                                            | See [GitLab documentation](https://docs.gitlab.com/install/self_compiled/#adding-your-trusted-proxies) for details |
...
| `workhorse.adoptCfRayHeader`                                  | `false`                                                         | Adopt incoming `Cf-Ray` header as Correlation-ID if present. See the [Workhorse documentation](https://docs.gitlab.com/development/workhorse/configuration/#propagate-correlation-ids) for more details. |
...
| `workhorse.healthcheckListener.enabled`                       | `false`                                                         | Enable the workhorse healthcheck listener and disables the default Puma readiness probe. Allows to detect the readiness status more reliably and with less flakyness. Introduced in GitLab 18.5. |
| `workhorse.healthcheckListener.port`                          | `8182`                                                          | Port number to use for the healthcheck listener. |
| `workhorse.healthcheckListener.pumaControl`                   | `true`                                                          | Query the Puma control application instead of the Puma readiness endpoint. |
| `workhorse.healthcheckListener.checkInterval`                 | `10s`                                                           | Time interval between consecutive health status checks of the upstream Puma server. |
| `workhorse.healthcheckListener.timeout`                       | `5s`                                                            | Timeout of Puma check requests. |
