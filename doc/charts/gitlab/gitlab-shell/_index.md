...
| `opensshd.supplemental_config`                           |                                                         | Supplemental configuration, appended to `sshd_config`. Strict alignment to [man page](https://manpages.debian.org/trixie/openssh-server/sshd_config.5.en.html) |
...
Example, enabling older clients using the `ssh-rsa` key exchange algorithms. Note that enabling deprecated algorithms, such as `ssh-rsa`, creates [significant security vulnerabilities](https://www.openssh.org/txt/release-8.8). The likelihood of exploitation is **significantly amplified** on publicly exposed GitLab instances with these changes.
