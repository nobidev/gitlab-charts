...
| `cgroups.initContainer.enabled`                          | `true`                                                  | When `true`, the `init-cgroups` init container will run to make the cgroup path writable. With `containerd` v2.1.0 or newer, a [new configuration option](https://github.com/containerd/containerd/issues/10924) allows to make the cgroup path writable directly from the container runtime. If that option is used, the `init-cgroups` init container can be disabled. Defaults to `true` for backward compatibility. |
...
| `cgroups.mountpoint`                                     |                                                         | Where the parent cgroup directory is mounted. When unset, it defaults to the pod cgroup path computed by the `init-cgroups` container if `cgroups.initContainer.enabled` is `true`, or to `/sys/fs/cgroup` if the init container is disabled. |
...
You can't change these through our settings once you've deployed. In
...
> TLS for communicating with it, refer
> [the external Gitaly documentation](../../../advanced/external-gitaly/_index.md#connecting-to-external-gitaly-over-tls).
