...
cgroup ownership so that Gitaly can manage cgroups. The init container:

- Must run as root (`runAsUser: 0`).
- Mounts a volume with write access to `/sys/fs/cgroup`.

The following example oversubscribes repository cgroups, assigning them more combined
resources than the parent cgroup limit, so individual repositories can burst without
raising the overall limit. For more information, see
[Configuring oversubscription](https://docs.gitlab.com/administration/gitaly/cgroups/#configuring-oversubscription).
...
The following example runs `init-cgroups` as a privileged container to satisfy an
