...
will be handled by the primary Redis instance specified by `global.redis.host`.
The only exception is for the [GitLab agent server (KAS)](gitlab/kas/_index.md),
which looks for Redis configuration in the following order:
...
### Disabling the Container Registry

There are two independent settings:

- `global.registry.enabled` controls the Container Registry integration: whether the
  GitLab application talks to a registry at all. This is the setting the application uses, for example
  when deleting or transferring projects.
- `registry.enabled` controls whether the **bundled Registry deployment** is created.

To fully disable the Container Registry features, set `global.registry.enabled: false`. Leaving the
integration enabled while no registry is reachable causes operations such as project deletion and
transfer to fail with errors like
`Failed to open TCP connection to gitlab-registry:5000`
(see [issue 24110](https://gitlab.com/gitlab-org/gitlab/-/issues/24110)). To also stop deploying the
bundled Registry, additionally set `registry.enabled: false`:
...
To use an external registry instead of the bundled one, set `registry.enabled: false` (no bundled
deployment) while keeping `global.registry.enabled: true` and pointing `global.registry.host` and
`global.registry.api` at the external registry:
