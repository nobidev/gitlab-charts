...
In version 9.9.0, the `GITLAB_SIDEKIQ_MAX_REPLICAS` environment variable was set by default in the Sidekiq chart. This unintentionally triggered the usage of a Sidekiq concurrency limiter which is not yet ready for use in self-managed environments. See [issue #393](https://gitlab.com/gitlab-com/gl-infra/tenant-scale/tenant-services/team/-/work_items/393).

A fix has been implemented in version 9.11.0 and will be backported in version 9.10.4. 

If you have already upgraded to a 9.10.x patch version below 9.10.4;
1. In the Sidekiq subchart under [extraEnv](../charts/gitlab/sidekiq/_index.md#extraenv), set `GITLAB_SIDEKIQ_MAX_REPLICAS: 0`.
2. Contact [GitLab Support](https://about.gitlab.com/support/). 

### Upgrade to 9.9.x

In version 9.9.0, the `GITLAB_SIDEKIQ_MAX_REPLICAS` environment variable was set by default in the Sidekiq chart. This unintentionally triggered the usage of a Sidekiq concurrency limiter which is not yet ready for use in self-managed environments. See [issue #393](https://gitlab.com/gitlab-com/gl-infra/tenant-scale/tenant-services/team/-/work_items/393).

A fix has been implemented in version 9.11.0 and will be backported in version 9.9.6. 

If you have already upgraded to a 9.9.x patch version below 9.9.6;
1. In the Sidekiq subchart under [extraEnv](../charts/gitlab/sidekiq/_index.md#extraenv), set `GITLAB_SIDEKIQ_MAX_REPLICAS: 0`.
2. Contact [GitLab Support](https://about.gitlab.com/support/). 
...
As part of the GitLab 18.0 release, [support for PostgreSQL 14 and 15 was deprecated](https://handbook.gitlab.com/handbook/engineering/data-engineering/database-excellence/database-frameworks/postgresql-upgrade-cadence/).
...
Please check the [Prometheus 3 migration guide](https://prometheus.io/docs/prometheus/latest/migration/),
...
- If you enabled the bundled `kube-state-metrics`, `alertmanager`, node exporter
