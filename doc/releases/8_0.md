...
See [GitLab 17 upgrade notes](https://docs.gitlab.com/update/versions/gitlab_17_changes/) for all upgrade relevant changes.
...
[external NGINX chart](../advanced/external-ingress/_index.md),
...
When upgrading to the mentioned chart versions, by default the `v1.11.2` will be set. The chart will automatically fallback to `v1.3.1` if you're setting
`nginx-ingress.rbac.create` to `false` (same applies to Geo `nginx-ingress-geo.rbac.create`). This is because `v1.11.2` requires new RBAC rules, which we added to our
...
or [re-enable the legacy workflow](https://docs.gitlab.com/administration/settings/continuous_integration/#control-runner-registration).
