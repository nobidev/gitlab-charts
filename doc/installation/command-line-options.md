...
| `registry.enabled`                                  | `true`                                      | Deploy the bundled Container Registry. This only controls the Registry deployment; the GitLab&nbsp;↔&nbsp;Registry integration is controlled by `global.registry.enabled`. |
...
| `global.registry.enabled`                                  | `true`                                                          | Enable the GitLab&nbsp;↔&nbsp;Registry integration used by the GitLab application (for example, when deleting or transferring projects). Set to `false` to fully disable Container Registry features. This is independent of `registry.enabled`, which only controls the bundled Registry deployment. |
