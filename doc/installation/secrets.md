...
Existing values are never overwritten. If you create a secret by hand, the chart keeps the
values you set and only adds keys that are missing. That behavior lets an installation pick
up secrets introduced in a later chart version.
...
For the mechanism that generates these secrets and how to select it, see the
[shared-secrets job](../charts/shared-secrets.md).
