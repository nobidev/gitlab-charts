...
Check the [external NGINX Ingress](nginx.md) to configure a prepare a external NGINX
Ingress deployment to be used with GitLab.

### Traefik

Traefik must be configured to expose port 22 for GitLab Shell (GitLab's SSH daemon):
