...
allows to control the buffering behavior of incoming clients.

The proxy request buffering is enabled by default, which means NGINX will buffer all incoming
requests and will only forward them to the backend after the full request was received.

If disabled, the request body is sent to the proxied server immediately as it is received.
