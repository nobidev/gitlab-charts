...
#   QA_EE_ACTIVATION_CODE=…     EE license activation code. REQUIRED for
...
# QA_EE_ACTIVATION_CODE feeds the gitlab-license Secret (cmd_deploy) and is
# consumed by gitlab-qa (cmd_qa). Without it the deploy still completes but
# EE-only test specs silently skip or fail at runtime — fail fast instead.
require_ee_license() {
  [[ -n "${QA_EE_ACTIVATION_CODE:-}" ]] \
    || die "QA_EE_ACTIVATION_CODE is unset — export it (or load from your password manager) before running deploy/qa"
}
...
cmd_deploy() {
  ...
  log "creating ${release}-gitlab-license Secret (empty if no QA_EE_ACTIVATION_CODE)"
