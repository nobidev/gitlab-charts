...
function deploy_chart() {
  ...
  # Populates charts/ from Chart.lock, skipping the rebuild when the cached
  # copy already matches. Runs from PROJECT_ROOT: the script resolves
  # Chart.lock and the chart dir relative to the working directory.
  (cd "${PROJECT_ROOT}" && "${SCRIPT_DIR}/ensure_chart_deps.sh")
  prepare_values
