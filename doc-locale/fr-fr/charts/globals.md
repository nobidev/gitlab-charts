...
> [!note]
> Les propriétés `bucket`, `enabled` et `proxy_download` sont les seules propriétés qui doivent être configurées au niveau de chaque élément (`global.appConfig.artifacts.bucket`, ...) si vous souhaitez déroger aux valeurs par défaut.
...
> [!note]
> Ces paramètres n'affectent pas les charts tiers intégrés.
...
> [!note]
> N'utilisez pas `global.serviceAccount.create=true` avec `global.serviceAccount.name`, car cela demande aux charts de créer plusieurs objets ServiceAccount avec le même nom. À la place, utilisez `global.serviceAccount.create=false` si vous spécifiez un nom global.
...
> [!note]
> Les charts maintenus en externe ne respectent pas `global.nodeSelector` pour le moment et pourraient nécessiter une configuration séparée en fonction des valeurs de chart disponibles. Cela inclut Prometheus, cert-manager, Redis, etc.
...
> [!note]
> L'implémentation ne prend pas en charge la réutilisation d'un nom de valeur avec différents types de contenu. Vous pouvez remplacer le même nom avec un contenu similaire, mais ne mélangez pas les sources comme `secretKeyRef`, `configMapKeyRef`, etc.
