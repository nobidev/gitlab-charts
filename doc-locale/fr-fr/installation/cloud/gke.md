...
- Installez le [Google SDK](https://cloud.google.com/sdk/docs/install).
...
Le script lit divers paramètres à partir des variables d'environnement et de l'argument `up` pour le bootstrap ou `down` pour le nettoyage.
...
| `CLUSTER_VERSION`     | Par défaut GKE, consultez les [notes de release GKE](https://cloud.google.com/kubernetes-engine/docs/release-notes) | La version de votre cluster GKE. |
...
| `PREEMPTIBLE`         | `false`                           | Moins cher, les clusters vivent au *maximum* 24 h. Aucun SLA sur les nœuds/disques. |
| `PROJECT`             | Pas de valeur par défaut, doit être défini.  | L'ID de votre projet GCP. |
| `RBAC_ENABLED`        | `true`                            | Si vous savez si RBAC est activé sur votre cluster, définissez cette variable. |
| `REGION`              | `us-central1`                     | La région où se trouve votre cluster. |
| `SUBNETWORK`          | default                           | Le sous-réseau à utiliser dans ce cluster. |
| `USE_STATIC_IP`       | `false`                           | Crée une IP statique pour GitLab au lieu d'une IP éphémère avec un DNS géré. |
| `ZONE_EXTENSION`      | `b`                               | L'extension (`a`, `b`, `c`) du nom de zone où se trouvent les instances de votre cluster. |

Exécutez le script en passant les paramètres souhaités. Il peut fonctionner avec les paramètres par défaut, à l'exception de `PROJECT` qui est obligatoire :
...
Deux ressources doivent être créées dans GCP : un cluster Kubernetes et une IP externe.
...
- Nous recommandons un cluster avec au moins 2 nœuds, chacun disposant de 4 vCPU et 15 Go de RAM.
- Notez la région du cluster, elle sera nécessaire à l'étape suivante.

#### Création de l'IP externe {#creating-the-external-ip}

Une IP externe est requise pour que votre cluster soit accessible. L'IP externe doit être régionale et se trouver dans la même région que le cluster lui-même. Une IP globale ou une IP en dehors de la région du cluster **not work**.

Pour créer une IP statique, exécutez :
...
Nous utiliserons cette IP pour la lier à un nom DNS dans la section suivante.

## Entrée DNS {#dns-entry}

Si vous avez créé votre cluster manuellement ou utilisé l'option `USE_STATIC_IP` lors de la création via un script, vous aurez besoin d'un domaine public avec une entrée DNS générique de type A pointant vers l'IP que nous venons de créer.
...
Poursuivez avec l'[installation du chart](../deployment.md) une fois que le cluster est opérationnel et que l'IP statique et l'entrée DNS sont prêtes.
