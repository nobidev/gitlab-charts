...
> [!note] Si vous activez la prise en charge des domaines personnalisés pour GitLab Pages, `external-dns` ne fonctionnera plus pour le domaine Pages (`pages.<global.hosts.domain>` par défaut). Vous devez configurer manuellement l'entrée DNS pour pointer le domaine vers l'adresse IP externe dédiée à Pages.
...
> [!note] Après le déploiement initial, toute modification de vos paramètres de stockage nécessite de modifier manuellement les objets Kubernetes. Il est donc préférable de planifier à l'avance avant de déployer votre instance de production afin d'éviter tout travail supplémentaire de migration de stockage.
