...
> [!note] Consultez les [termes définis](https://docs.gitlab.com/administration/geo/glossary/) pour décrire tous les aspects de Geo (principalement la distinction entre `site` et `node`).
...
1. Déployez le chart en utilisant cette configuration :
   ...
   > [!note] Cela suppose que vous utilisez l'espace de nommage `gitlab`. Si vous souhaitez utiliser un espace de nommage différent, vous devez également le remplacer dans `--namespace gitlab` tout au long du reste de ce document.
