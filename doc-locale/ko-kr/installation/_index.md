...
> [!note]
> GitLab Helm 차트는 외부 PostgreSQL, Redis 및 오브젝트 스토리지가 필요합니다. 프로덕션 환경의 경우 [클라우드 네이티브 하이브리드 참조 아키텍처](#use-the-reference-architectures)를 따르세요.
...
> [!warning]
> 업그레이드의 일부로 무중단을 달성하는 것은 분산 애플리케이션의 경우 특히 어렵습니다. 이 문서는 우리의 HA 참조 아키텍처와 비교하여 주어진 대로 테스트되었으며 효과적으로 관찰 가능한 다운타임이 없었습니다. 그러나 특정 시스템 구성에 따라 결과가 다를 수 있다는 점을 유의하세요.
>
