...
> [!warning] 시크릿 `RELEASE-gitlab-initial-root-password`을 수동으로 삭제하지 않으면 다음 릴리스에서 재사용됩니다. 노출된 방식에 관계없이 이 비밀번호를 수동으로 삭제해야 합니다. 예를 들어 기록된 데모에서입니다. 이는 노출된 비밀번호가 향후 릴리스에서 인스턴스에 로그인하는 데 사용될 수 없음을 보장합니다.
