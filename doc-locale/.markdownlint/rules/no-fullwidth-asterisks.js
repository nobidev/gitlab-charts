    ...
    lines.forEach((line, lineIndex) => {
      ...
      while ((match = fullwidthPattern.exec(line)) !== null) {
        ...
        const isInInlineCode = inlineCodeRanges.some(range => {
          ...
          // Find all backtick pairs and check if match falls within any of them
          const backtickRegex = /`([^`]+)`/g;
          let codeMatch;
          while ((codeMatch = backtickRegex.exec(line)) !== null) {
            if (codeMatch[1] === range.content) {
              const codeStart = codeMatch.index;
              const codeEnd = codeMatch.index + codeMatch[0].length;
              if (matchPos >= codeStart && matchPos <= codeEnd) {
                return true;
              }
            }
          }
